...
*/
		...
		std::set<int>* pivotOuts;
		pivotOuts = new std::set<int>();
		std::set<int>* bestSharedOuts;
		bestSharedOuts = new std::set<int>();
		int prevscore =  0;
		...
		std::copy(outEdges[v]->begin(),outEdges[v]->end(),std::inserter(*pivotOuts,pivotOuts->begin()));
	 ...
	 for(edgeiter = first; edgeiter!=last;edgeiter++)
		{
			...
			int u = *edgeiter;
            int udeg = degmapout[u];
			printf("top nbor: %d with  %d\n",u,udeg);
			if(udeg>0)
			{
				//move to bottom of heap
				 std::set<int>* newset = outEdges[u];
		    printf("got edges for u: %d degmap: %d\n",newset->size(),udeg);
			 std::set<int> newintersect;
			 std::set_intersection(pivotOuts->begin(),pivotOuts->end(),newset->begin(),newset->end(),std::inserter(newintersect,newintersect.begin()));
			printf("made new intersect.\n");
			 int sharednborsize = newintersect.size();
			 printf("intersection size: %d\n",sharednborsize);
			 if(sharednborsize>maxOverlap)
			 {
				 printf("beats old  record of %d\n",maxOverlap);
				 maxOverlap = sharednborsize;
			    bestSharedOuts->clear();
				bestSharedOuts = new std::set<int>();
				 std::copy(newintersect.begin(),newintersect.end(),std::inserter(*bestSharedOuts,bestSharedOuts->begin()));
				 bestSender = u;
			 }
	...
	//				printf("new receiver set: size = %d\n",newintersect.size());
		...
		printf("best sender count: %d=%d\n",maxOverlap,bestSharedOuts->size());
	    if(maxOverlap>0)
		{
			usedSenders->push_back(v);
			usedSenders->push_back(bestSender);
			int score = maxOverlap - 1;
			cumulativeScore+=score;

			printf("used senders: %d %d overlap %d score %d added to get %d\n",v,bestSender,maxOverlap,score,cumulativeScore);
	  	...
	  	printf("cumulativescore: %d\n",cumulativeScore);
			...
			for (auto it = bestSharedOuts->begin(); it != bestSharedOuts->end(); ++it)
			{
				int receiver = *it;
