...
//  int tempProduct = degmapout[rankedSenders->at(i1+1)]*degmapout[rankedSenders->at(i1+2)];
  int vertsused = 0;
  NborSet topout1 = *outdegheap.begin();
  std::set<NborSet, nbor_count_compare>::iterator topoutit=outdegheap.begin();
  topoutit++;
  NborSet topout2 = *topoutit;
  int top1 = topout1.rec;
  int top2 = topout2.rec;
  printf("start main loop: initial degs: %d %d \n",degmapout[top1],degmapout[top2]);
...
*/
		//new -------
		usedSenders->push_back(top1);
		usedSenders->push_back(top2);
		printf("using top1,top12: %d,%ld\n",top1,top2);
		//		printf("using i1,i2=(%d,%d) = (%d,%d)\n",i1,i2,rankedSenders->at(i1),rankedSenders->at(i2));

	  i1++;
	  i2=i1+1;
	 topout1 = *outdegheap.begin();
 	 topoutit=outdegheap.begin();
 	 topoutit++;
 	 topout2 = *topoutit;
     top1 = topout1.rec;
     top2 = topout2.rec;
