...
{
	//main loop: add a pair to the fixed choices set each round
	//itset comes in as all "start" - score graph ignores past the k we give it
	for(int i =0; i<k; i++)
	{
		cout<<"iterativeSearch round "<<i<<endl;
		int iterMax = 0;
		//iterator  for searching for new item this round:
	   std::unordered_map<std::pair<int,int> , int>::iterator bestiter= counter.begin();
	    cout<<"nothing in pairs? "<<(bestiter==counter.end())<<endl;
		if(bestiter==counter.end()) return 0;
	   while(itset[i]!=counter.end())
		{
			// create itset of fixedset+option off list of new options
		   int score =makeAndScoreGraph(i+1,itset,sharedOuts);
			cout<<"iterativeSearch tested: ";
		   printItset(k,itset);
		   cout<<"score: "<<score<<endl;
			if (score>iterMax)
		    {
				cout<<"iterativeSearch score wins as opt"<<score<<endl;
				iterMax = score;
				bestiter = itset[i];
			}
			else if(score==iterMax){
				//ordered comparison between itset and bestiter - pick lower?
				int  currentFirst = itset[i]->first.first;
				int currentSecond = itset[i]->first.second;
				int bestFirst = bestiter->first.first;
				int bestSecond = bestiter->first.second;
				if(currentFirst<bestFirst || (currentFirst==bestFirst && currentSecond< bestSecond))//use current if alphabetically lower
				{
					bestiter =itset[i];	
				}
			}
			else{ cout<<"iterativeSearch score doens't win"<<score<<endl;}
			itset[i]++;
		}
		itset[i] = bestiter;//save our  winner for next round
	    cout<<"iterativeSearch final for i="<<i<<" ";
		printItset(k,itset);
	}
	return makeAndScoreGraph(k,itset,sharedOuts);
}
int recursiveSearch(
		int iterindex,
		int k,
...
int matchingscore=  iterativeSearch(k,itset,sharedOuts,counter);
